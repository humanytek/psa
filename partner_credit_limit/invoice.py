...
class account_invoice(osv.osv):
    ...
    def check_limit_credit(self, cr, uid, ids, context={}):
        ...
        if invoice.type == "out_invoice" and ((saldo_maturity + invoice.amount_total) > partner.credit_maturity_limit or (saldo + invoice.amount_total) > partner.credit_limit):
            if not partner.over_credit :
                ...
                self.pool.get('res.partner').write(cr, uid, [partner.id], {'credit_limit':credit - debit + invoice.amount_total})
            ...
            if not partner.maturity_over_credit:
                ...
                self.pool.get('res.partner').write(cr, uid, [partner.id], {'credit_maturity_limit':credit_maturity - debit_maturity + invoice.amount_total})
                return True
        else:
            return True
account_invoice()
