...
class invoice_facturae_html(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            ...
            'get_serial_number': self._get_serial_number,
            'get_partner_bank_ref':self.get_partner_bank_ref,
            'get_uos_qty': self.get_qty_product
        })
        self.taxes = []
    ...
    def _get_serial_number_from_picking(self, picking_objs):
        for picking_obj in picking_objs:
            for move_line in picking_obj.move_lines:
                if not self.product_serial_dict.get(move_line.product_id.id, False):
                    self.product_serial_dict[move_line.product_id.id] = []
                if move_line.prodlot_id:
                    self.product_serial_dict[move_line.product_id.id].append(move_line.prodlot_id.name)
        return True
    
    def _get_serial_number(self, line):
        product_serial = line.production_lot_id and line.production_lot_id.name or ''
        
#         if not self.product_serial_dict and line.invoice_id:
#             if line.invoice_id.picking_id:
#                 self._get_serial_number_from_picking([line.invoice_id.picking_id])
#             elif line.invoice_id.sale_id:
#                 self._get_serial_number_from_picking(line.invoice_id.sale_id.picking_ids)
#         product_id = line.product_id.id
#         if self.product_serial_dict.get(product_id, False):
#             product_serial_list = self.product_serial_dict[product_id]
#             product_serial_list = list(set(product_serial_list))
#             product_serial = ', '.join(product_serial_list)
        return product_serial
    
    def get_partner_bank_ref(self, partner_id):
#         import pdb
#         pdb.set_trace()
        partner_pool = self.pool.get('res.partner')
        partner_obj = partner_pool.browse(self.cr, self.uid, partner_id.id)
        bank_ref = partner_obj.bank_reference
        return bank_ref
    
    def get_qty_product(self, invoice_id):
        pool = pooler.get_pool(self.cr.dbname)
        invoice_obj = pool.get('account.invoice')
        invoice = invoice_obj.browse(self.cr, self.uid, invoice_id)
        uos_qty = invoice.qty_uos
        return uos_qty
        

webkit_report.WebKitParser('report.sfs.account.invoice.facturae.webkit.inherit',
            'account.invoice',
            'addons/sfs_l10n_mx_facturae_customization/report/invoice_facturae_html.mako',
